...
pickle_files = [f for f in os.listdir(pickle_dir) if f.endswith('.gpickle')]
    ...
    try:
        with open(first_file, 'rb') as f:
            ...
            if isinstance(data, nx.Graph):
                ...
                nodes = list(data.nodes(data=True))
                if nodes:
                    print(f"Node attributes (first node): {nodes[0]}")
                    
                    # Check for risk_score
                    scores = [d.get('risk_score', 'N/A') for n, d in nodes]
                    print(f"Risk Scores Sample: {scores[:5]}")
